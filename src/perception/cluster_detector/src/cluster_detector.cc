...
void cloud_toolbox::ClusterDetector::cloud_preprocess(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, 
    ...
    std::cout<< "Cloud primitive " << cloud->points.size() << " points." <<std::endl;
    ...
    std::cout<< "Cloud after in range filter " << cloud_inrange->points.size() << " points." <<std::endl;
    ...
    std::cout<< "Cloud after voxel grid filter " << cloud_voxel->points.size() << " points." <<std::endl;
    


    // changing
    // add statistical oulier removal filter => remove snow 
    pcl::StatisticalOutlierRemoval<pcl::PointXYZI> sor_filter;
    sor_filter.setInputCloud(cloud_voxel);
    sor_filter.setMeanK(mean_k_);
    sor_filter.setStddevMulThresh(std_dev_mul_thresh_); // bigger remove more outliers
    sor_filter.setNegative(false);
    sor_filter.filter(*filtered_cloud);



    std::cout<< "Cloud after statistical oulier removal filter " << filtered_cloud->points.size() << " points." <<std::endl;
    
    std::cout<<"remove outlier number : "<<cloud_voxel->points.size() - filtered_cloud->points.size() << std::endl;
    std::cout<<std::endl;
...
void cloud_toolbox::ClusterDetector::ground_filter(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, 
    ...
    for (int i = 0; i< iter_ground_plane_; i ++){
        ...
        // DEBUGGING
        // std::cout<< "th_distance "<< th_distance_<<std::endl;
        
        // std::cout<< "th_ground_plane "<< th_ground_plane_<<std::endl;
        // std::cout<< "distance "<< distance<<std::endl;
        
        // -0.2 ??     0.1 ?? => 0.3      negative number ??       
