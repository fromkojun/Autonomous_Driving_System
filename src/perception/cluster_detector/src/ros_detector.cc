...
cloud_toolbox::ROSDetector::ROSDetector(ros::NodeHandle &nh){
    ...
    nh.getParam("voxel_filter_leaf_size_z", voxel_filter_leaf_size_[2]);
        ...
        // statistical filter 
    nh.getParam("mean_k", mean_k_);
    nh.getParam("std_dev_mul_thresh", std_dev_mul_thresh_);
...
void cloud_toolbox::ROSDetector::cal_occ_grid(pcl::PointCloud<pcl::PointXYZI>::Ptr scene_cloud, nav_msgs::OccupancyGrid& occ_grid){
    int grid_cell_width = 700;
    int grid_cell_height = 700;
    ...
    for (size_t i = 0; i < scene_cloud->points.size(); i ++){
        ...
        int grid_x = (-1 * scene_cloud->points[i].y + map_center_x) / g_resolution;
        int grid_y = (scene_cloud->points[i].x + map_center_y) / g_resolution;
    ...
    // changing 
    occ_grid.info.origin.position.x = (-1) * (grid_cell_height / 2.0) * g_resolution;
    occ_grid.info.origin.position.y = (grid_cell_width / 2.0) * g_resolution;
    occ_grid.info.origin.position.z = 0.0;
    

    // changing
    tf::Quaternion q;
    q.setRPY(0,0, -90 * 3.1415926 / 180.0 );
...
void cloud_toolbox::ROSDetector::publish_ros(){
    ...
    //changing / occ_grid => marker_array
    marker_array_publisher.publish(marker_array);
    
    
    //changing / occ_grid => boundary_marker
